...
class Emulate:
    def __init__(
        ...
        frequencies=None):
        """
        The emulator class for building, training, and using 21cmLSTM to emulate ARES signals
        The default parameters are for training/testing 21cmLSTM on the ARES set described in Section 2.2 of DJ+24
        ...
        Methods
        -------
        load_model : load an existing instance of 21cmLSTM trained on ARES data
        ...
        self.emulator = model(self.signal_train.shape[-1], self.par_train.shape[-1]+1, 1,
                              activation_func, name="emulator_ARES")
    ...
    def load_model(self, model_path=PATH+"models/emulator_ARES.h5"):
        """
        Load a saved model instance of 21cmLSTM trained on the ARES data set.
        The instance of 21cmLSTM trained on ARES included in this repository is a representative trial
    ...
    def train(self, epochs, batch_size, callbacks=[], verbose=2, shuffle='True'):
        """
        Train an instance of 21cmLSTM on the ARES data set
        ...
        self.emulator.save(PATH+'models/emulator_ARES_trialx.h5') # save the entire model in HDF5 format

        return train_loss, val_loss

    def predict(self, params):
        """
        Predict global 21 cm signal(s) from input ARES physical parameters using trained instance of 21cmLSTM
        ...
        emulated_signals = np.squeeze(emulated_signals, axis=2)
    ...
    def test_error(self, relative=True, nu_low=None, nu_high=None):
        """
        Compute the rms error for the loaded trained instance of 21cmLSTM evaluated on the 1,704-signal ARES test set
