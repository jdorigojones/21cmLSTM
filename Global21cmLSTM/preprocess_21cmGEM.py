...
def preproc_params(unproc_params: np.ndarray, train_params: np.ndarray) -> np.ndarray:
    ...
    np.savetxt('train_mins_21cmGEM.txt', train_data_mins)
    np.savetxt('train_maxs_21cmGEM.txt', train_data_maxs)
    return proc_params
...
def preproc_signals(unproc_signals: np.ndarray, train_signals: np.ndarray) -> np.ndarray:
    ...
    with open("train_mins_21cmGEM.txt", "a") as f:
       f.write(str(train_signals_min))
    with open("train_maxs_21cmGEM.txt", "a") as file:
