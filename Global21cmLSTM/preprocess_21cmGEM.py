...
def preproc_params(unproc_params: np.ndarray, train_params: np.ndarray) -> np.ndarray:
    ...
    #np.savetxt('models/train_mins_21cmGEM.txt', train_data_mins) # these lines are for saving the min and max values of the training set 
    #np.savetxt('models/train_maxs_21cmGEM.txt', train_data_maxs) # used for signal denormalization; uncomment if using different data
    return proc_params
...
def preproc_signals(unproc_signals: np.ndarray, train_signals: np.ndarray) -> np.ndarray:
    ...
    # with open("models/train_mins_21cmGEM.txt", "a") as f: # uncomment if using different data
    #    f.write(str(train_signals_min))
    # with open("models/train_maxs_21cmGEM.txt", "a") as file: # uncomment if using different data
