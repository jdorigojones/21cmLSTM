...
### Background
21cmLSTM (Dorigo Jones et al. 2024, or DJ+24) is a long short-term memory (LSTM) recurrent neural network (RNN) emulator of the global 21 cm signal. 21cmLSTM has unprecedented emulation accuracy because it leverages the intrinsic correlation between neighboring frequency bins (i.e., autocorrelation) in the 21 cm signal, which is a result of the unique ability of LSTM RNNs to learn patterns in sequential data. Given input astrophysical parameters, 21cmLSTM outputs, or predicts, a realization of the global 21 cm signal across redshifts z=5-50 (spanning the EoR to the Dark Ages).
