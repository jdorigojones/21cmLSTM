...
class evaluate():
    def __init__(self, **kwargs):
        ...
        self.train_mins = np.loadtxt(self.model_dir+'train_mins.txt')
        self.train_maxs = np.loadtxt(self.model_dir+'train_maxs.txt')
        
        self.model = kwargs.pop('model', None)
        if self.model is None:
            self.model = keras.models.load_model(self.model_dir+'emulator.h5',compile=False)
